...
AIGC Gallery 是一款完全在浏览器中运行的本地 AI 图片管理工具，无需账号、后端或 API Key。图片与图库记录保存在当前浏览器的 IndexedDB 中，设置保存在本地；图片导入、元数据解析、编辑和备份均在本机完成，不上传到服务器。启动时会向 GitHub 请求一次最新版本号用于提示更新，该请求不携带任何图库数据，可在「关于 → 设置」中关闭。
...
| 画师串画廊 | 按自定义分组管理画师串，保存示例图、自定义风格分类、标签、收藏和评分，复制提示词，导入 / 导出 JSON 资料 |
...
先安装 [Node.js](https://nodejs.org/)（包含 npm），推荐 **Node.js 24 LTS**，最低要求 **22.12**。

下载并解压源码，或克隆仓库后，可以直接使用根目录的快捷启动脚本：

- **Windows**：双击 `start.bat`。
- **macOS / Linux**：在仓库目录的终端运行 `bash start.sh`。

脚本会检查运行环境，首次运行或依赖清单变化时自动执行 `npm ci`，需要联网。后续启动复用已安装依赖；缺少 Vite 入口或更换 Node.js 主版本时会重新安装。安装失败时会停止并显示错误，不会继续启动。

服务就绪后会自动打开 [http://localhost:5173](http://localhost:5173)。使用期间保留终端窗口，结束时按 **Ctrl+C**；Windows 若询问是否终止批处理，输入 `Y` 并回车。端口占用时脚本会报错退出，不会切换端口或终止其他进程；若已运行的是本项目，可直接打开现有页面。Windows 出错时窗口会保留，便于查看提示。macOS / Linux 的图形环境若未自动打开浏览器，可手动访问上述地址。

脚本用于**从源码运行**；发布包中的 `index.html` 仍可直接打开，无需安装 Node.js。也可以手动运行：
...
导入图片时，「从提示词自动提取标签」默认关闭。使用逗号分隔的标签式提示词时，可在 **关于 → 设置** 开启，仅影响之后导入的图片。
...
| `npm run test:launcher` | 运行启动脚本的隔离测试，不安装依赖或访问实际图库 |
